...
public class MimicFollower : MonoBehaviour {
	...
	private void SpeedUp(){
		...
		else if (currentProlong.z_plus > 0) {
	...
	void OnTriggerEnter(Collider col){
		if (col.GetComponent<Prolongation> () != null && currentProlong == null) {
			...
			tileStart = currentProlong.x_start;
			tileEnd = currentProlong.x_end;
			prolongStart = currentProlong.x_prolongStart;
			prolongEnd = currentProlong.x_prolongEnd;
			print ("in");
		}
	}
	...
	void OnTriggerExit(Collider col){
		if (col.GetComponent<Prolongation> () != null && col.GetComponent<Prolongation>() == currentProlong) {
			if (currentProlong.x_plus > 0) {
				...
				p = Mathf.Clamp01 (p);
				print ("exit " + p);
				float x_offset = 1f * (prolongEnd - prolongStart) * currentProlong.x_plus / (currentProlong.x_plus + 1f);
				...
				p = Mathf.Clamp01 (p);
				print ("exit " + p);
				float z_offset = 1f * (prolongEnd - prolongStart) * currentProlong.z_plus / (currentProlong.z_plus + 1f);
				if (p == 0f && currentProlong.addedToOffset == true) {
